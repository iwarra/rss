...
export default async function Home({
  ...
  try {
    ...
    const [articlesPage, availableFeeds] = await Promise.all([
      getArticles({ ...query, limit: DEFAULT_ARTICLE_PAGE_SIZE }),
      getFeeds(),
    ]);
  ...
  return (
    <main className={styles.page}>
      ...
      <ArticleFilters feedNames={feedNames} values={filterValues} />
      {error ? (
        ...
        <p className={styles.message}>
          ...
          <a
            href={`/?${serializeArticleListQuery(filterValues, pagination.totalPages)}`}
          >
      ...
      {!error && pagination && pagination.page <= pagination.totalPages && (
        <ArticlePagination filters={filterValues} pagination={pagination} />
      )}
    </main>
  );
}
